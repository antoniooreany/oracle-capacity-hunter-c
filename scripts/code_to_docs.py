...
def build_payload(owner: str, repo: str, pr_number: int) -> Payload:
    ...
    title = f"{type_label.split(':', 1)[1]}: update PR #{pr_number} docs".replace("feat", "docs")
    ...
    risks_lines = [
        ...
        "- No application runtime behavior is changed in this workflow.",
        "",
    ]

    config_lines = [
        "## Config",
        "- Requires `GITHUB_TOKEN`/`GH_TOKEN` in the environment.",
        "- Relies on GitHub REST API for pull request files.",
        "",
    ]
    ...
    testing_lines = [
        ...
        "- Workflow should be exercised on a test PR using `[review-pr]` comment.",
        "",
    ]

    body_md = "\n".join(
        summary_lines + changes_lines + risks_lines + config_lines + testing_lines
    )
